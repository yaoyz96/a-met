"""
For pre-training (encoder with linear layer):
>>> python run.py --config <path to config file> --manner whole --gpu 0

For meta-training (encoder only):
>>> python run.py  config <path to config file> --manner episode --gpu 0
"""
...
class Learner:
  def __init__(self, resume, mode, manner, config):
    ...
    if self.mode != "test":
      if self.manner == 'whole':
        self.val_model = init_model(device, "episode-val", "meta-"+config["model"], **config['model_args'])
        self.val_model.backbone = self.model.backbone
      else:
        self.val_model = self.model

    if self.mode == 'train_test':
      if self.manner == "whole":
        self.test_model = init_model(device, "episode-test", "meta-"+config["model"], **config['model_args'])
      else:
  ...
  def run(self, config):
    ...
    start_epoch = -1
    if self.resume:
      resume_model = os.path.join(config["checkpoint_dir"], None)
      helpers.print_and_log("\033[0;32mINFO\033[0m |==> Resume training from {}".format(resume_model))

      self.model, 
      self.optimizer, 
      self.lr_scheduler, 
      start_epoch, config = helpers.load_state(self.model, self.optimizer, self.lr_scheduler, resume_model)
  ...
  def train_val(self, start_epoch, config):
    ...
    if config['_pre_model'] is not None:
      ...
      self.model = helpers.load_model(self.model, pre_ckp)
    ...
    if self.manner == "whole":
      baseloop = BaseLoop(self.model) 
      metaloop = MetaLoop(self.val_model, metric)
    else:
      baseloop = metaloop = MetaLoop(self.model, metric)
    ...
    for epoch in range(start_epoch+1, config['epoch']):
      ...
      train_loss, train_acc = baseloop.train_loop(self.optimizer, self.train_loader, iter_num)
      ...
      log_str = '\033[0;35m[Epoch {}]\033[0m \033[0;36mtrain\033[0m loss/acc {:.2f}|{:.2f}'.format(epoch, train_loss, train_acc)
  ...
  def test(self, checkpoint_dir, config):
    ...
    for tn in test_list:
      ...
      metaloop = MetaLoop(model, metric)
      acc, conf = [], []
